...
1. **软件性质**  
   本软件系影视聚合客户端（空壳），**不内置 VOD（点播）源或 IPTV 源**，相关数据源由用户自行添加和配置。

   电台功能接入第三方服务，其可用性可能随服务调整而变化。
...
| 架构分层      | 主要技术栈                                            |
| ------------- | ----------------------------------------------------- |
| 🖥️ 桌面运行时 | Tauri 2、Rust                                         |
| 🛠️ 构建与分发 | Vite、Tauri CLI、官方 updater 插件                    |
| 🗄️ 数据层     | SQLite、SQLx（Rust）                                  |
...
在设置的数据管理中导出或恢复完整备份，备份文件为独立的 SQLite `.db` 文件，可在 Windows 与 macOS 之间迁移数据。

本次 Tauri 重写发布后的旧版本备份会先在临时副本中升级；如果备份来自更新版本，请先升级应用。恢复前自动保留当前数据的安全备份，原备份文件不会被修改。
