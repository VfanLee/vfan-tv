...
/** 注册 IPTV 源管理、文件导入导出、目录与播放 IPC 处理器 */
export function registerIptvSourcesIpc(context: ApplicationContext): void {
  const { iptvSource, iptvCatalog, iptvPlayback, mediaPlaybackTarget } = context.services
...
async function logIptvRequest<T>(
  ...
  const networkLabel = context.services.network.getRouteDescription('iptv')
