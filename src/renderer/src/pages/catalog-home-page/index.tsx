        ...
        <section className="mt-6">
          ...
          {hierarchy.children.length > 0 && hierarchy.selectedParent ? (
            <div className="mt-3 flex flex-col gap-2">
              <span className="text-muted-foreground text-sm font-medium">{hierarchy.selectedParent.name}</span>
              <SegmentedTabs
...
function CatalogSearchForm({
  ...
  return (
    <form
      className="border-input bg-background focus-within:border-ring flex h-12 items-center gap-3 rounded-xl border px-4 transition-colors"
      role="search"
      onSubmit={(event) => {
        event.preventDefault()
        onSubmit(value)
    ...
    >
      <Search className="text-muted-foreground shrink-0" size={19} />
      <input
        aria-label="搜索当前点播源"
      ...
      {value ? (
        <button
          aria-label="清除搜索"
          className="text-muted-foreground hover:bg-accent hover:text-foreground focus-visible:ring-ring flex size-8 items-center justify-center rounded-lg outline-none focus-visible:ring-2"
          type="button"
          onClick={() => {
            setValue('')
            onClear()
      ...
      <button
        className="bg-primary text-primary-foreground hover:bg-primary/90 focus-visible:ring-ring rounded-lg px-4 py-2 text-sm font-bold outline-none focus-visible:ring-2"
        type="submit"
      >
        搜索
      </button>
...
function CatalogMessage({
  ...
  return (
    <div className="border-border bg-card/60 flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed px-6 text-center">
      <Icon className="text-primary" size={30} />
      <h3 className="mt-4 text-lg font-bold">{title}</h3>
      <p className="text-muted-foreground mt-2 max-w-md text-sm leading-6">{description}</p>
      {action && onAction ? (
        <button
          className="bg-primary text-primary-foreground hover:bg-primary/90 focus-visible:ring-ring mt-5 rounded-xl px-4 py-2.5 text-sm font-bold outline-none focus-visible:ring-2"
          type="button"
          onClick={onAction}
        >
          {action}
...
function NoSourceState({
  ...
  return (
    <div className="text-foreground flex min-h-full items-center justify-center bg-transparent px-6">
      <div className="border-border bg-card max-w-md rounded-3xl border p-8 text-center shadow-sm">
        <Clapperboard className="text-primary mx-auto" size={34} />
        <h1 className="mt-5 text-2xl font-bold">还没有可用的点播源</h1>
        <p className="text-muted-foreground mt-3 text-sm leading-6">
          {errorMessage || '前往设置添加点播源，或启用一个已有的点播源后再回来。'}
        </p>
        <button
          className="bg-primary text-primary-foreground hover:bg-primary/90 focus-visible:ring-ring mt-6 inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-bold outline-none focus-visible:ring-2"
          type="button"
          onClick={onOpenSettings}
        >
          <Settings2 size={17} /> 打开设置
        </button>
      </div>
    </div>
  )
