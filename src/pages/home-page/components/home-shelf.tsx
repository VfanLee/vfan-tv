...
import type { RecentPlayItem, RecommendationItem } from '@/types'
import { EmptyState, MediaPoster, PosterCardSkeleton, PosterPlayOverlay } from '@/components'

/** 渲染首页内容栏骨架屏 */
export function HomeShelfSkeleton(): React.JSX.Element {
  return (
...
/** 渲染最近播放卡片 */
export function RecentPlayCard({
  item,
  onClick,
  onDelete,
}: {
...
export function EmptyShelf({
  ...
  return <EmptyState density="compact" description={description} icon={icon} title={title} />
}

/** 渲染首页星级评分 */
function HomeStarRating({ item }: { item: RecommendationItem }): React.JSX.Element | null {
  const filledStars = getFilledStars(item)
  if (filledStars === undefined) return null
  return (
    <div
...
function getFilledStars(item: RecommendationItem): number | undefined {
  if (typeof item.ratingStarCount === 'number' && item.ratingStarCount > 0) {
    ...
    return clamp(Math.round(starCount), 0, 5)
  }
  if (typeof item.rating === 'number' && item.rating > 0) return clamp(Math.round(item.rating / 2), 0, 5)
  return undefined
}
