...
import { BasicPlayer, MediaPoster } from '@/components'
import { useMediaPlaybackTarget } from '@/hooks'
...
export function VodPage(): React.JSX.Element {
  ...
  const selectSource = (item: VodSearchResult): void => {
    ...
    const preferredLineIndex = clamp(activeSelection.lineIndex, 0, Math.max(0, targetLines.length - 1))
    const targetLine = targetLines[preferredLineIndex]
    const preferredEpisodeIndex = clamp(
