...
import { addFavorite, isApiAvailable, isFavorite as checkIsFavorite, removeFavorite } from '@/platform/api'
...
export function useVodFavorite(current: VodSearchResult | undefined, resourceKey: string): VodFavoriteState {
  ...
  const toggle = async (): Promise<void> => {
    if (!current || (!isDesktopRuntime() && !isApiAvailable()) || isLoading) return
    setIsLoading(true)
    try {
  ...
  /** 加载当前点播内容的收藏状态 */
  useEffect(() => {
    if (!current || (!isDesktopRuntime() && !isApiAvailable())) return
    let active = true
