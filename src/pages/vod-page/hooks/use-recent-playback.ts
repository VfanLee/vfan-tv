...
import type { PlayLine, VodSearchResult } from '@/types'
import { isApiAvailable, upsertRecentPlay } from '@/platform/api'
...
export function useRecentPlayback(
  ...
  const save = async ({
    ...
    if (!current || !activeLine || !activeEpisode || (!isDesktopRuntime() && !isApiAvailable())) return
    const now = Date.now()
