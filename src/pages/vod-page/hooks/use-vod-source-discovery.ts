...
export function useVodSourceDiscovery({
  ...
  const refreshSources = useCallback(async (): Promise<void> => {
    if ((!isDesktopRuntime() && !isApiAvailable()) || !current?.title || isRefreshingSources) return
    ...
    try {
      const result = await searchVod(current.title, (id) => {
        refreshSearchIdRef.current = id
      })
      if (!result) setIsRefreshingSources(false)
    } catch (error) {
      refreshSearchIdRef.current = undefined
      setIsRefreshingSources(false)
      toast.error('搜索同名资源失败', { description: String(error) })
    }
