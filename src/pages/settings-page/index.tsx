...
import { ConfirmDialog, LayoutPreferencesSettings, SettingsPageLayout } from '@/components'
import { isApiAvailable, onSettingsSectionChange } from '@/platform/api'
...
export function SettingsPage(): React.JSX.Element {
  ...
  const sourceApiAvailable = apiAvailable || isDesktopRuntime()
  const vod = useVodSources(sourceApiAvailable)
  const iptv = useIptvSources(sourceApiAvailable)
  const general = useGeneralSettings({
    apiAvailable: sourceApiAvailable,
    refreshIptvSources: iptv.refresh,
    refreshVodSources: vod.refresh,
  })
  const network = useNetworkSettings(sourceApiAvailable)
        ...
        <div className="min-w-0 pb-8">
          ...
          {activeSection === 'data-management' ? (
            <SettingsPageLayout title="数据管理">
              ...
              />
              <DiagnosticsSettingsCard apiAvailable={sourceApiAvailable} />
            </SettingsPageLayout>
          ) : null}
