...
import type { NetworkProxyTestResult, NetworkRouteKey, NetworkSettings, NetworkStatus } from '@/types'
import {
...
const ROUTE_LABELS: Record<NetworkRouteKey, string> = {
  iptv: 'IPTV 直播网络',
}
...
export function useNetworkSettings(apiAvailable: boolean): NetworkSettingsState {
  ...
  useEffect(() => {
    ...
    void Promise.all([getSettings(), getNetworkStatus()])
      ...
      .catch((error: unknown) => {
        if (active) toast.error('加载网络配置失败', { description: String(error) })
      })
      .finally(() => {
