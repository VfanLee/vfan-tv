...
import type { SubscriptionConfig, SubscriptionNetworkMode } from '@/types'
import { deleteSourceSubscription, getSettings, syncSourceSubscription, updateSettings } from '@/platform/api'
...
export function useGeneralSettings({
  ...
  useEffect(() => {
    ...
    void getSettings()
      ...
      .catch((error: unknown) => {
        if (active) toast.error('加载订阅配置失败', { description: String(error) })
      })
    return () => {
      active = false
    }
