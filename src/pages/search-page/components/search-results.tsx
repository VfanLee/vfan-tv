...
import type { SearchSourceStatus, VodSearchResult } from '@/types'
import { EmptyState, MediaPoster } from '@/components'
...
export function SearchEmptyState(): React.JSX.Element {
  return (
    <EmptyState
      description="搜索后可以按影片分组查看，也可以切到单个数据源确认返回状态。"
      icon={Search}
      title="输入关键词开始搜索"
    />
  )
...
function ResultPlaceholder({ isSearching }: { isSearching: boolean }): React.JSX.Element {
  return (
    <EmptyState
      description={isSearching ? '多个源会增量出现，先搜到的内容会先展示。' : '换个关键词再试一次。'}
      icon={isSearching ? Loader2 : AlertCircle}
      iconClassName={isSearching ? 'animate-spin' : undefined}
      title={isSearching ? '正在等待数据源返回' : '还没有可展示的结果'}
    />
  )
}
