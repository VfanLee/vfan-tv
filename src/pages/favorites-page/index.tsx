...
import { listFavorites, onAppDataChange, removeFavorite } from '@/platform/api'
import { favoriteToVodSearchResult } from '@/platform/playback'
    ...
    <div className="text-foreground min-h-full bg-transparent px-10 py-9 pr-24">
      <div className="w-full">
        ...
        {items.length > 0 ? (
          ...
          <EmptyState
            description={isLoading ? '正在加载收藏内容…' : '在点播页收藏喜欢的内容后会显示在这里。'}
            icon={isLoading ? Loader2 : Heart}
            iconClassName={isLoading ? 'animate-spin' : undefined}
            title={isLoading ? '正在加载收藏' : '还没有收藏内容'}
          />
        )}
      </div>
      {pendingDeleteItem ? (
