import { readCachedSelection, writeCachedSelection } from './utils'
...
import { BasicPlayer, type PlayerRuntimeInfo } from '@/components'
import {
  getIptvCatalog,
  ...
  useEffect(() => {
    ...
    void getIptvCatalog(sourceId)
      .then((catalog) => {
        ...
        const previous = readCachedSelection(sourceId)
        const remembered =
          previous?.channelId === channelId ? selected.streams.find((item) => item.id === previous.streamId) : undefined
        setStreamId(remembered?.id ?? selected.streams[0].id)
      })
      .catch((error: unknown) => {
  ...
  useEffect(() => {
    ...
    void getIptvPlaybackTarget(sourceId, channel.id, stream.id)
      .then((nextTarget) => {
        ...
        writeCachedSelection(sourceId, { channelId: channel.id, streamId: stream.id, expandedGroups: [channel.group] })
